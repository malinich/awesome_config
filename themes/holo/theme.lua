...
local bat = lain.widget.bat({
    settings = function()
        bat_header = " Bat "
        bat_p      = bat_now.perc .. " ▼ "
        --bat_p      = os.execute("upower -i /org/freedesktop/UPower/devices/battery_BAT0 | grep capacity | awk '{ print $2 }' | xargs | grep -E '^\w*[^,]' -o")
...
function theme.table_conn()
  return awful.key({ "Shift"         }, "Shift_R", function() kbdcfg:next() end )  
  -- return awful.key({ "Mod1" }, "t", function() kbdcfg:next() end )
end
